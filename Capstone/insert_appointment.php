            ...
            <div class="col-sm-10">
                ...
                <div class="row">
                    ...
                    <div class="card-group" style="padding:1%;">
                        ...
                        <?php
                            ...
                            if(isset($_POST["date"]))
                            {
                                $Date = $_POST["date"];
                                $Today = date("m-d-Y");
                                $Time1 = DateTime::createFromFormat('m-d-Y', $Date)->getTimestamp(); 
                                $Time2 = DateTime::createFromFormat('m-d-Y', $Today)->getTimestamp();
                                $pass = $_POST["password"];
                            ...
                            if($Checking["Pass"]==$pass)
                            {
                                if($valid)
                                {
                                    if(isset($_POST["timeslots"]))
                                    {
                                        ...
                                        if($Timeslots > 0)
                                        {
                                            ...
                                            isset($_POST["LName"]) && 
                                            isset($_POST["patientTimes"]) && 
                                            isset($_POST["patientLocation"]))
                                            {
                                                $FName = $_POST["FName"];
                                                $LName = $_POST["LName"];
                                                $Sessions = $_POST["sessions"];
                                                $Valid = True;
                                                $Hygen = $_POST["Appt_Type"];
                                                ...
                                                $StartTime = date($TimeFormat, strtotime($_POST["patientTimes"]));
                                                $NextSession = date("h:i A", strtotime($StartTime ." +30 MINUTE"));

                                                $PatientLocation = $_POST["patientLocation"];
                                                

                                                for($Session = 0; $Session<$Sessions+1; $Session++)
                                                {
                                                    
                                                    $sql = "INSERT INTO Patients(PatientFNAME, PatientLNAME, APPDATE, PatientTIME, PatientLOCATION, PatientPN, HYGIENIST)
                                                    values (\"" . $FName . "\",\"" . $LName . "\",\"" . $Date . "\",\"" . $StartTime . "\",\"" . $PatientLocation . "\",\"" . $PatientPN ."\",\"". $Hygen . "\")";
                                                    $result = mysqli_query($conn, $sql);
                                                    if(!$result)
                                                    {
                                                        echo "<h5>Invalid Entry!!!!</h5>";
                                                        break;
                                                    }
                                                ...
                                                $sixmonth = "INSERT INTO sixmonthreminder(PatientFNAME, PatientLNAME, APPDATE, PatientTIME, PatientLOCATION, PatientPN)
                                                values (\"" . $FName . "\",\"" . $LName . "\",\"" . $Date . "\",\"" . $PatientTime . "\",\"" . $PatientLocation . "\",\"" . $PatientPN ."\")";
                                                
                                                $result2 = mysqli_query($conn, $sixmonth);
                                                
                                                if($result && $result2)
                                                {
                                                    echo "<h5>Success! " . $FName . " " . $LName . " has an appointment set on " . $Date . " at " . $PatientTime . " until " . $StartTime . "</h5>";
                                                }
                                                else
                                                {
                                                    echo "<h5>Error!! Missing an information please try again!</h5>";
                                                }
                                            }
                                                
                                            else
                                            {
                                                Echo "<h5>Error Missing an Information from the input Please try again!</h5>";
                                            }
                                        }
                                        else if($Timeslot < 9)
                                        {
                                            Echo "<h5>Error! Max Allocated time has been used! Please try again!</h5>";
                                        }
                                        else
                                        {
                                            Echo "<h5>Error! Time cannot be negative! Please try again!</h5>";
                                        }
                                    }
                                    else
                                    {
                                        if(isset($_POST["FName"]) && isset($_POST["LName"]) && isset($_POST["patientTimes"]) && isset($_POST["patientLocation"]))
                                        {
                                            $FName = $_POST["FName"];
                                            $LName = $_POST["LName"];
                                            $Hygen = $_POST["Appt_Type"];
                                            ...
                                            $PatientTime = date("h:i A", strtotime($_POST["patientTimes"]));
                                            $PatientLocation = $_POST["patientLocation"];
                                            $sql = "INSERT INTO Patients(PatientFNAME, PatientLNAME, APPDATE, PatientTIME, PatientLOCATION, PatientPN, HYGIENIST)
                                            values (\"" . $FName . "\",\"" . $LName . "\",\"" . $Date . "\",\"" . $PatientTime . "\",\"" . $PatientLocation . "\",\"" . $PatientPN ."\",\"". $Hygen . "\")";
                                            
                                            
                                            $sixmonth = "INSERT INTO sixmonthreminder(PatientFNAME, PatientLNAME, APPDATE, PatientLOCATION, PatientPN)
                                            values (\"" . $FName . "\",\"" . $LName . "\",\"" . $Date . "\",\"" . $PatientLocation . "\",\"" . $PatientPN ."\")";
                                            
                                            $result2 = mysqli_query($conn, $sixmonth);
                                            $result = mysqli_query($conn, $sql);
                                            if($result && $result2)
                                            {
                                                echo "<h5>Success! " . $FName . " " . $LName . " has an appointment set on " . $Date . " at " . $PatientTime . "</h5>";
                                            }
                                            else
                                            {
                                                echo "<h5>Error!!! Missing an information please try again!</h5>";
                                ...
                                {
                                    echo "<h5>Error! Cant schedule an appointment in the past!</h5>";
                                }
                            }
                            else
                            {
                    ...
                    <form action="insert_appointment.php" method="post">
                        ...
                        <p for="time" >Appointment Time: <div id="times"></div></p>
                        <br>
                        <p>Location: <input type="string" name="patientLocation" value="Portsmouth" placeholder="Location" required readonly></p>
                        <p>Patient Phone Number: <input type="tel" id= "patientPN" value= "<?php echo $_POST["patientPN"]?>" name="patientPN" placeholder="Patient Phone Number" pattern="[0-9]{3}[0-9]{3}[0-9]{4}" onkeypress = 'return keypresshandlerNUM(event)'></p>
                        ...
                        <div id="Sessions" style="display:none">
                            <p>Add more session. Each session is 30 mins <input type="number" value = "1" id="sessions" name="sessions" palceholder="1" min="0" max="9"></p>
                        </div><br>
                        <div>
