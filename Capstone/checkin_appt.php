...
<html lang="en">
<head>
    <title>Checking in Appointment</title>
    <meta charset="utf-8">
...
<body>
    ...
    function CheckinList($conn)
    {
        $Today = date('m-d-Y');
        $sql = "SELECT PatientFNAME, PatientLNAME, PatientTIME, PATSTATUS FROM patients where APPDATE= \"" . $Today . "\"";
        ...
        <div class="row">
            ...
            <div class="col-sm-10">
                ...
                <div class="row">
                            ...
                            <table id="intercard_table_table" class="table table-sm table-bordered table-striped">
                                <tbody>
                                    <?php
                                        ...
                                        $Today = date('m-d-Y');
                                        $sql = "SELECT Patientid, PatientFNAME, PatientLNAME, PatientTIME, PATSTATUS FROM patients where APPDATE= \"" . $Today . "\" where PatientLOCATION = 'Portsmouth'";
                                        $result = mysqli_query($conn, $sql);
                                        $rowcount = mysqli_num_rows($result)-1;
                                        $count = 0;
                                        ...
                                        if(mysqli_num_rows($result) > 1) 
                                        {
                                            ...
                                            for($i = 0; $i <= $rowcount; $i++)
                                            {
                                                ...
                                                {
                                                    $stat = $_POST[$i];
                                                    if($stat != $ArrayofStatus[$ArrayofIndex[$i]])
                                                    {
                                                        $sqlupdate = "UPDATE Patients set PATSTATUS = \"" . $stat . "\" where Patientid = " . $ArrayofIDs[$ArrayofIndex[$i]];
                                        ...
                                        else if (mysqli_num_rows($result) == 1)
                                        {
                                            ...
                                            if($_POST['0'] != $input["PATSTATUS"])
                                            {
                                                $sqlupdate = "UPDATE Patients set PATSTATUS = \"" . $input["PATSTATUS"] . "\" where Patientid = " . $input["Patientid"];
                                                mysqli_query($conn, $sqlupdate);
                                                CheckinList($conn);
                                            }
                                        }
                    ...
                    <input class="btn btn-primary" type="submit" name="submitAdd" id="submitAdd" value="Update Schedule">
                    </form>
